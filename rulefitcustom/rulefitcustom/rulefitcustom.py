...
class RuleCondition():
    ...
    def transform(self, X):
        ...
        """
        if self.operator == "<=":
            res = 1.0 * (X[:,self.feature_index] <= self.threshold)
        elif self.operator == ">":
            res = 1.0 * (X[:,self.feature_index] > self.threshold)
        return res
...
class Rule():
    ...
    def transform(self, X, weigh_rules=False):
        """Transform dataset.

        Parameters
        ----------
        X: array-like matrix
        weigh_rules: Boolean. Applies a weight according to the conditions size
        ...
        if (weigh_rules):
            return reduce(lambda x,y: x * y / len(self.conditions), rule_applies)
        else:
...
class RuleEnsemble():
    ...
    def transform(self, X, coefs=None, weigh_rules=False):
        """Transform dataset.
        ...
        if   coefs is None :
            return np.array([rule.transform(X, weigh_rules=weigh_rules) for rule in rule_list]).T
        else: # else use the coefs to filter the rules we bother to interpret
            res= np.array([rule_list[i_rule].transform(X, weigh_rules=weigh_rules) for i_rule in np.arange(len(rule_list)) if coefs[i_rule]!=0]).T
...
class RuleFitCustom(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if 'r' in self.model_type:
            ...
            X_rules = self.rule_ensemble.transform(X, weigh_rules = self.simple_rules)
    ...
    def predict(self, X):
        ...
        if 'r' in self.model_type:
            rule_coefs=self.coef_[-len(self.rule_ensemble.rules):] # bug correction. upstreamed at https://github.com/christophM/rulefit/issues/23
            if len(rule_coefs)>0:
                X_rules = self.rule_ensemble.transform(X,coefs=rule_coefs, weigh_rules = self.simple_rules)
                if X_rules.shape[0] >0:
                    X_concat = np.concatenate((X_concat, X_rules), axis=1)
    ...
    def get_rules(self, exclude_zero_coef=False):
        ...
        for i in range(0, n_features):
            if self.lin_standardise:
                ...
                coef=self.coef_[i]
            output_rules += [(self.feature_names[i], 'linear',coef, 1, 0)] # TODO REMOVE, pour debug
        ## Add rules
        for i in range(0, len(self.rule_ensemble.rules)):
            rule = rule_ensemble[i]
            coef=self.coef_[i + n_features]
            output_rules += [(rule.__str__(), 'rule', coef, rule.support, i)] # TODO REMOVE, pour debug
        rules = pd.DataFrame(output_rules, columns=["rule", "type", "coef", "support", "rule_number"]) # TODO REMOVE, pour debug
